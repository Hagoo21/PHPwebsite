...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "INSERT INTO property (street, city, province, postalCode, type, beds, bath, cost, apartmentNum, dateListed, parking, access, laundry, managerID, code) 
              VALUES (:street, :city, :province, :postalCode, :type, :beds, :bath, :cost, :apartmentNum, :dateListed, :parking, :access, :laundry, :managerID, 0)";
    $statement = $pdo->prepare($query);
    $statement->execute([
        ':street' => $street,
        ':city' => $city,
    ...
    // Insert data into ownsProperty table using session variable
    $ownerID = $_SESSION['user_id']; // Assuming you have a 'user_id' session variable
    $query = "INSERT INTO ownsProperty (ownerID, propertyID) VALUES (:ownerID, :propertyID)";
    $statement = $pdo->prepare($query);
    $statement->execute([
        ':ownerID' => $ownerID,
    ...
    // If the property type is House, Apartment, or Room, create a new instance in the respective table
    switch ($type) {
        case 'House':
            $query = "INSERT INTO house (code) VALUES (:code)";
            break;
        case 'Apartment':
            $query = "INSERT INTO apartment (code) VALUES (:code)";
            break;
        case 'Room':
            $query = "INSERT INTO room (code) VALUES (:code)";
            break;
        default:
    ...
    if (isset($query)) {
        $statement = $pdo->prepare($query);
        $statement->execute([':code' => $propertyID]);
    }
